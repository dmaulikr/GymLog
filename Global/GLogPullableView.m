...
@implementation GLogPullableView
...
- (void)handleDrag:(UIPanGestureRecognizer *)sender {
    ...
    if ([sender state] == UIGestureRecognizerStateBegan) {
        ...
        CGFloat scale = 1.0 - translate.y / 160.0;
        self.transform = CGAffineTransformScale(originalTransform, scale, scale);      // self.center = newPos;
...
- (void)setOpened:(BOOL)op animated:(BOOL)anim {
    ...
    if (anim) {
        ...
        if ([delegate respondsToSelector:@selector(GLogPullableView:didChangeState:)]) {
            [delegate GLogPullableView:self didChangeState:opened];
        }
    }
}
